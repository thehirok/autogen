...
\input{tables/tab_feature_importance.tex}
...
\input{tables/tab_selection_accuracy.tex}
...
\input{tables/tab_significance.tex}
...
\input{tables/tab_equivalence.tex}
...
We compare three meta-models, MLP, Random Forest, and XGBoost, to determine which architecture is most suitable for this task. Results are shown in Table~\ref{tab:ablation}.

\input{tables/tab_ablation.tex}

The results show that MLP achieves the highest leave-one-out accuracy at 69\%, followed by Random Forest at 62\% and XGBoost at 54\%. However, with only 13 datasets in the meta-dataset, these differences correspond to just 1--2 datasets and should not be over-interpreted. All three models perform within a similar range, and the ranking could change with additional datasets.

The fact that MLP slightly outperforms the tree-based models is noteworthy, as neural approaches typically require more training data. In this case, the small feature space (29 features) may allow the MLP to learn useful decision boundaries even with limited training instances. Nevertheless, the differences are not statistically meaningful at this scale.
...
In future work, this approach can be extended to more datasets and different algorithm families to test whether the same observations hold in broader settings.
